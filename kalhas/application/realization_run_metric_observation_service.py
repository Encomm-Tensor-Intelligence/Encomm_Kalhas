"""Deterministic runtime-3 realization metric-observation extraction (Phase 25).

Extracts the immutable, tenant-scoped ``RealizationRunMetricObservationSet``
of one runtime 3.0.0 run from its **completely verified** stored
...
1. ``verify_run_trajectory_inputs`` (exactly once) loads and verifies
   every recorded trajectory input and resolves the exact applicable
   plan tuple, the closed compiled-world catalogs, and the exactly-once
   reconstructed ``WorldRealization``. Unknown or foreign runs fail with
   the store's typed not-found error; tampered inputs fail with the
   typed integrity error. The separate run-scoped verifier is never
   called here.
2. The recorded runtime version must be exactly ``"3.0.0"`` (legacy
   1.0.0, 2.0.0, and unsupported versions raise the typed
   unsupported-version error), the run must be COMPLETE (typed
   invalid-state error otherwise), and the verified realization must be
   present.
3. The stored ``RealizationRunTrajectoryExecution`` is loaded **only
   through the store boundary** (a fresh deep copy) and fully verified
   with the existing authoritative runtime-3 integrity pipeline
   (``verify_realization_run_trajectory_execution_record``) before any
   final state is read.
4. The observation bindings are read from the exact verified compiled
   ...
   state-model/field provenance against the embedded records (and the
   metric exactly once in the embedded ``ScenarioSpec``), locates
   exactly one matching ``RealizedStateTrajectoryResult`` (missing and
   ambiguous results are rejected), requires exact state-model
   identifier and content-hash agreement, extracts **only**
   ``final_state[state_field_id]`` (never ``initial_state``), validates
   the raw value against the authoritative integer/number kind
   (booleans, strings, and non-finite values rejected, no coercion,
   exact int/float representation preserved), copies the metric unit
   from the embedded ``ScenarioSpec``, and binds the value to the exact
   result/plan/execution provenance.
5. The complete immutable observation set is built with the
   deterministic runtime-3 identifier, the verified world/strategy/seed/
   realization/execution provenance, ``observed_at`` equal to the
   authoritative execution's ``executed_at`` (never the wall clock), and
   the content hash computed over the complete canonical payload
   excluding ``content_hash``. It is stored **only after every
   validation and integrity check succeeds** - any failure writes
   nothing; a second extraction is rejected with the typed
   already-exists error and never overwrites the stored artifact.

The verifier (``verify_realization_run_metric_observation_set_record``)
resolves the authoritative inputs (when not supplied), loads and fully
verifies the authoritative execution, strictly revalidates the stored
contract, re-derives the deterministic identifier, regenerates the
expected set in memory, and requires exact **canonical JSON** equality -
which distinguishes int/float representation and avoids Python's
``True == 1`` value-kind confusion. It never repairs, normalizes,
reorders, overwrites, or silently accepts a partial artifact. The read
service returns a stored set only after that full verification and
never creates an artifact when none exists.

Nothing here evaluates or re-executes transitions, rebuilds or repairs
an execution, triggers replay, reads ``initial_state`` as an
observation, samples or consumes uncertainty, invokes LEGION or NEXUS,
loads/imports/instantiates/executes domain packs, performs network,
provider, filesystem, or database operations, or uses randomness or
wall-clock time. No probabilities, risk statistics, rankings, or
...
def _reject(run_id: str, reason: str) -> RealizationRunMetricObservationIntegrityError:
    """A generic, safe integrity error with an internal diagnostic reason."""
    return RealizationRunMetricObservationIntegrityError(run_id, reason)
...
def _verify_binding_provenance(
    ...
    authoritative model field. Returns the authoritative metric unit
    copied from the embedded ``ScenarioSpec``.
    """
...
def _extract_raw_value(
    ...
    *,
    ...
    exactly match the authoritative numeric kind - booleans, strings,
    non-numeric values, and non-finite floats are rejected, and no
    coercion ever happens. The extracted value is preserved exactly: no
    normalization, scaling, transformation, or unit conversion, and the
    exact int/float representation is kept.
    """
...
def build_realization_run_metric_observation_set(
    *,
    ...
    Requires the realization trajectory runtime version and a COMPLETE
    run; reads the observation bindings only from the verified compiled
    world and extracts one raw value per binding from the verified
    execution's exact final state, in canonical ``metric_id`` order. The
    execution must agree with the verified records on run, world content
    hash, input hash, runtime literal, strategy, seed, campaign/run-plan
    identity, and the exact reconstructed realization. The set
    ...
    if verified.run_plan.runtime_version != REALIZATION_TRAJECTORY_RUNTIME_VERSION:
    ...
    if inputs.realization is None:
        raise _reject(run_id, "realized initial state missing after trajectory verification")

    if execution.run_id != run_id:
        raise _reject(run_id, "realization execution run identity mismatch")
    if execution.tenant_id != verified.run_plan.tenant_id:
        raise _reject(run_id, "realization execution tenant mismatch")
    if execution.campaign_id != verified.run_plan.campaign_id:
        raise _reject(run_id, "realization execution campaign mismatch")
    if execution.run_plan_id != verified.run_plan.identifier:
        raise _reject(run_id, "realization execution run plan mismatch")
    if execution.world_version_id != verified.world.identifier:
        raise _reject(run_id, "realization execution world version mismatch")
    if execution.world_content_hash != verified.world.content_hash:
        raise _reject(run_id, "realization execution world content hash mismatch")
    if execution.input_hash != verified.run_plan.input_hash:
        raise _reject(run_id, "realization execution input hash mismatch")
    if execution.runtime_version != REALIZATION_TRAJECTORY_RUNTIME_VERSION:
        raise _reject(run_id, "realization execution runtime version mismatch")
    if execution.strategy_candidate_id != verified.strategy.identifier:
        raise _reject(run_id, "realization execution strategy mismatch")
    if execution.strategy_content_hash != strategy_candidate_content_hash(verified.strategy):
        raise _reject(run_id, "realization execution strategy content hash mismatch")
    if execution.scenario_seed_id != verified.seed.identifier:
        raise _reject(run_id, "realization execution scenario seed mismatch")
    if execution.world_realization_id != inputs.realization.identifier:
        raise _reject(run_id, "realization execution realization identity mismatch")
    if execution.world_realization_content_hash != inputs.realization.content_hash:
        raise _reject(run_id, "realization execution realization content hash mismatch")
    ...
    observation_set = RealizationRunMetricObservationSet(
        identifier=realization_run_metric_observation_set_identifier(
        ...
        world_realization_id=inputs.realization.identifier,
        world_realization_content_hash=inputs.realization.content_hash,
...
def _verified_extraction_inputs(
    *,
    ...
    1.0.0, 2.0.0, and unsupported recorded versions raise the typed
    unsupported-version error; runs that are not COMPLETE raise the
    typed invalid-state error; a missing verified realization is
    rejected. Read-only: nothing is evaluated, recorded, or changed, and
    the realization is never reconstructed or resampled here.
    """
...
def extract_realization_run_metric_observations(
    *,
    store: InMemoryScenarioStore,
    tenant_id: str,
    run_id: str,
    ...
    inputs verified (exactly once), runtime/completeness gates applied,
    the stored ``RealizationRunTrajectoryExecution`` loaded through the
    store boundary and fully verified, every binding extracted against
    the verified world and execution, and the complete set hashed. Only
    then is the set stored; any failure writes nothing. A second
    extraction of the same run is rejected with the typed already-exists
    error and never overwrites the stored artifact.
    """
    ...
    observation_set = build_realization_run_metric_observation_set(
        inputs=trajectory_inputs, execution=execution
    )
    store.put_realization_run_metric_observation_set(tenant_id, run_id, observation_set)
    return observation_set


def verify_realization_run_metric_observation_set_record(
    ...
    *,
    ...
    bindings, and ``RealizationRunTrajectoryExecution``, regenerates the
    expected observation set in memory, and requires exact canonical
    JSON equality - identifier, ownership, ordering, values,
    provenance, and content hash - so int/float representation and
    boolean-value confusion are detected. The stored artifact is never
    repaired, normalized, reordered, overwritten, or silently accepted;
    any mismatch raises the safe typed integrity error with a generic
    public message.
    """
    ...
    expected = build_realization_run_metric_observation_set(
        inputs=trajectory_inputs, execution=execution
    )
    ...
    if canonical_json(observation_set.model_dump(mode="json")) != canonical_json(
        expected.model_dump(mode="json")
    ):
        raise _reject(run_id, "metric observation set does not match the regenerated artifact")


def get_verified_realization_run_metric_observation_set(
    *,
    store: InMemoryScenarioStore,
    tenant_id: str,
    run_id: str,
    ...
    gates are verified (exactly once), the stored set is loaded through
    the store's deep-copy boundary (a missing or foreign set raises the
    typed not-found error and nothing is ever created), the
    authoritative execution is verified, and the stored set is returned
    only after full regeneration-equality verification. A corrupted or
    tampered set (or corrupted authoritative inputs) fails through the
    typed integrity mapping. This never performs extraction, evaluation,
    replay, or any write.
    """
    trajectory_inputs = _verified_extraction_inputs(store=store, tenant_id=tenant_id, run_id=run_id)
    stored = store.get_realization_run_metric_observation_set(tenant_id, run_id)
