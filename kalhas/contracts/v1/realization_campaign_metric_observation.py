"""Realization-aware campaign metric-observation matrix contracts (Phase 25).

Phase 25 introduces the **deterministic realization-aware campaign
metric-observation matrix**: the exact authoritative ``strategy candidate x
identical shared seed`` observation layout of one completed runtime-3.0.0
campaign, assembled from every completely verified Phase 25
``RealizationRunMetricObservationSet`` of that campaign into one immutable,
self-hashing, comparison-ready artifact of **exact raw observations and
provenance only** - the layout already proven by the Phase 25
``RealizationCampaignTrajectoryMatrix``.

The matrix mirrors the Phase 21 ``CampaignMetricObservationMatrix`` exactly
and adds the seed-aligned realization provenance: every cell carries the
world realization identity and content hash of its seed, and the aggregate
carries the complete ordered world-realization identity/hash tuples with
cell-to-tuple agreement enforced by the contract.

- ``RealizationCampaignMetricObservationCell`` is one authoritative
  campaign run of the matrix: its sequence, strategy, and seed positions;
  the run and run-plan identity; the strategy and seed identities; the run
  input hash; the verified runtime-3 trajectory-execution reference
  (deterministic identifier and content hash); the verified Phase 25
  ``RealizationRunMetricObservationSet`` reference (deterministic
  identifier and content hash); the world realization identity and content
  hash; and the exact ordered tuple of Phase 20 ``RunMetricObservationValue``
  records - the raw values are reused directly, never duplicated,
  converted, or interpreted.
- ``RealizationCampaignMetricObservationMatrix`` is the immutable
  aggregate: campaign, scenario, and world identity with the world content
  hash; the realization trajectory runtime version (always ``"3.0.0"``);
  the comparison mode (always ``identical_conditions``); the exact ordered
  strategy candidate identifiers, the exact ordered shared seed
  identifiers, the exact ordered metric identifiers, and the seed-aligned
  world-realization identity/hash tuples; the complete cell tuple in the
  ...
  clock. Its identifier is deterministic from the campaign identity, the
  world identity, and the runtime version.

The matrix is **comparison-ready raw observation provenance only**. It
performs no statistical aggregation of any kind, creates no
``MetricOutcome`` or ``OutcomeVector``, scores or ranks no strategy,
decides nothing about which strategy is better, and produces no evidence,
recommendations, or ``DecisionBrief``.
...
every cell's observations are empty); the seed-aligned realization tuples
exactly one entry per seed; the complete Cartesian product present exactly
once; every cell bound to its declared strategy and seed position; cells
in the exact RunPlan order; every cell's realization identity/hash
agreeing with the seed-aligned tuples; and every cell's observation metric
identifiers equal to ``ordered_metric_ids`` exactly and in the same order.
Authoritative identity, hash, and binding-provenance verification against
the stored records remains in the application layer (the pure matrix
builder and the verified query service).
...
REALIZATION_METRIC_OBSERVATION_MATRIX_RUNTIME_VERSION_LITERAL = "3.0.0"


class RealizationCampaignMetricObservationCell(BaseModel):
    """One authoritative campaign run of the realization-aware metric-observation matrix.
    ...
    trajectory-execution reference (deterministic identifier and content
    hash); the verified Phase 25 ``RealizationRunMetricObservationSet``
    reference (deterministic identifier and content hash); the world
    realization identity and content hash whose realized final states
    produced the observations; and the exact ordered tuple of Phase 20
    ``RunMetricObservationValue`` records - raw values and provenance
    ...
    target values, no strategy policy content, no outcomes, no evidence,
    no ranking or score, and no explanations or hidden reasoning.
    """
    ...
    realization_run_metric_observation_set_content_hash: Sha256Hex
...
class RealizationCampaignMetricObservationMatrix(VersionedContract):
    """The deterministic realization-aware metric-observation matrix of one completed
    3.0.0 campaign.
    ...
    world realizations, and every cell carries the verified Phase 25
    ``RealizationRunMetricObservationSet`` reference and the exact raw
    observation values of its run. The matrix is a comparison-ready raw
    observation and provenance artifact only - it never aggregates
    observations, calculates outcomes or distributions, scores or ranks
    strategies, decides that one strategy is better, or produces evidence,
    recommendations, or decision briefs. ``assembled_at`` is derived from
    the recorded campaign ``created_at`` - never the wall clock - and the
    ...
    cells: tuple[RealizationCampaignMetricObservationCell, ...] = Field(min_length=1)
    content_hash: Sha256Hex
    assembled_at: AwareDatetime

    @model_validator(mode="after")
    def _structural_matrix_shape(self) -> RealizationCampaignMetricObservationMatrix:
